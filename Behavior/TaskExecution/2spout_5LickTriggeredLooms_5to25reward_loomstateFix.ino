...
const int volumes[NUMBER_OF_TRIALS] = {
  0,0,0,0,0, //0uL additional
  1000, 1000, 1000, 1000, 1000, //10uL additional
  2000, 2000, 2000, 2000, 2000, 2000, 2000, 2000, 2000, 2000, //20ul additional
};
...
int prevtrialrewarded = 0;  //this is used as a logic gate to ensure that the mice continue to retreive rewards without looms
...
int loomstate = 0;          //logic gate to dictate whether a loom will be presented
int prevloomstate = 0;      //logic gate to dictate whether a loom will be presented
int loomupdate = 0;         //logic gate to dictate whehther to update the loomstate ## some of this seems redundant, but the code isn't working properly without it

int trialcount = 0;         //trial counter
int lickcount = 0;          //lick counter for determining whether the mouse is in an active lick bout
int loomcount = 0;          //loom counter
...
unsigned long prevlicktime = 0;     //timer to allow counting licks within a single lick bout
...
//  prevlick3state = lick3state;
  ...
  if (lick4state != prevlick4state) {
    if (digitalRead(lick4) == LOW) {
      ...
      if ((licknow == lick4) && (loomstate == 1)){ //automated, lick triggered looms
        if (licktime - prevlicktime <= 2000){
            lickcount += 1;
        }
        ...
        if (lickcount >= 4){
          loomcount += 1;
          Serial.println("LoomTrigger");
          Serial.print("Loom ");
          Serial.print(loomcount);
          Serial.print(" Presented during Trial ");
          Serial.print(trialcount);
          Serial.print(", spout ");
          ...
          lickcount = 0;
          loomstate = 0; //turn the loom state off, until it is reset based on successful trials and the necessary conditions (rewardVol and spout4)
          Serial.println("Loom State = 0"); //bug zapper
        }        
      }
    }
  }
  ...
  if ((restartstate == 1) && ((millis() - trialend) >= interval)) { 
    rewardlocation = selectRandomSpoutFromRemaining();
    restarttime = millis();
    restartstate = 2;
  }
...
//now looms are being triggered by licks, so no need for this line
// if (rewardlocation == 7) { 
...
//    Serial.print(" is active, ");
//    Serial.println(loomstart);
//    digitalWrite(ttl3out, HIGH);
//    delay(10);
//    digitalWrite(ttl3out, LOW);
//  }

if ((trialstate == 1) && (loomupdate == 1) && (rewardlocation == 4) && (rewardvol == 2000) && (trialcount >= 11) && (prevtrialrewarded == 1) && (prevloomstate == 0)) { //turn lick-triggered looms back on if last trial rewarded, rewardvol >2000, spout4 active 
    loomstate = 1;
    loomupdate = 0;
    Serial.println("Loom State = 1"); //bug zapper
  }
...
int selectRandomSpoutFromRemaining()
{
  const int spout_trials[] = {2,2,2,1};  //global variable
  return spout_trials[random(0, 4)];//select_spout, this is returning an index for the spout_trials[] list
}
